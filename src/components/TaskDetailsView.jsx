...
export default function TaskDetailsView({ task, user, sharedView, userColor, onToggleSubtask, onClose }) {
  ...
  const renderCreatorAssignee = () => (
    <div className="grid grid-cols-2 gap-2">
      ...
      <div>
        <label className="block text-[10px] font-medium text-gray-500 dark:text-gray-400 mb-0">{'\u{1F91D}'} Asignado a</label>
        <div className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-50 dark:bg-gray-800 rounded-lg">
          {task.assignee ? (
            <>
              <Avatar user={task.assignee} sizeClass="w-4 h-4 text-[8px]" fallbackClass="bg-emerald-400 text-white" />
              <span className="text-[11px] text-gray-700 dark:text-gray-300 font-medium">{task.assignee.name}</span>
            </>
          ) : (
            <span className="text-[11px] text-gray-400 dark:text-gray-500">Sin asignar</span>
          )}
        </div>
      </div>
    </div>
  ...
  const renderImages = () => {
    ...
    return (
      <div>
        <label className="block text-[10px] font-medium text-gray-500 dark:text-gray-400 mb-0">{'\u{1F4F7}'} Imágenes ({imgs.length})</label>
  ...
  return (
    <div className="space-y-2">
      {/* Título (read-only) */}
      <div>
        <label className="block text-[10px] font-medium text-gray-500 dark:text-gray-400 mb-0">Título</label>
      ...
      <div>
        <label className="block text-[10px] font-medium text-gray-500 dark:text-gray-400 mb-0">
      ...
      {!sharedView && (
        <>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="block text-[10px] font-medium text-gray-500 dark:text-gray-400 mb-0">Prioridad</label>
              <p className="text-sm px-3 py-1.5 bg-gray-50 dark:bg-gray-800 rounded-lg">{PRIORITIES.find((p) => p.value === task.priority)?.label || task.priority || 'Media'}</p>
            </div>
            <div>
              <label className="block text-[10px] font-medium text-gray-500 dark:text-gray-400 mb-0">Fecha límite</label>
              <p className="text-sm px-3 py-1.5 bg-gray-50 dark:bg-gray-800 rounded-lg">
          ...
          {/* Etiquetas */}
          <div>
            <label className="block text-[10px] font-medium text-gray-500 dark:text-gray-400 mb-0">Etiquetas</label>
            <p className="text-sm px-3 py-1.5 bg-gray-50 dark:bg-gray-800 rounded-lg">{task.tags || '—'}</p>
          </div>

          {/* Estado (completado) */}
          {(task.completedAt || task.status === 'DONE' || task.status === 'ARCHIVED') && (
            <div className="border-t border-gray-100 dark:border-gray-700 pt-2">
              <label className="block text-[10px] font-medium text-gray-500 dark:text-gray-400 mb-0">Estado</label>
              <p className="text-sm text-emerald-600 dark:text-emerald-400 font-semibold flex items-center gap-1">
                {'\u2705'} Completada {task.completedAt && (
                  <span className="text-xs text-gray-400 dark:text-gray-500 font-normal">
                    el {formatDateFull(task.completedAt)}
                  </span>
                )}
              </p>
            </div>
